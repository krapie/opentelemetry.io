...
저장소에 커밋이 발생할 때마다 부하 테스트(load test)를 실행한다. 이러한 부하
테스트는 테스트마다 다양한 구성 옵션으로 컬렉터의 바이너리를 실행하고 컬렉터를
통해 트래픽을 전송한다. 테스트 환경에 대한 추가 정보는
[저장소](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/testbed#opentelemetry-collector-testbed)에서
확인할 수 있다.

아래에는 결과의 일부가 표시된다. 전체 결과는
[컬렉터 벤치마크](https://open-telemetry.github.io/opentelemetry-collector-contrib/benchmarks/loadtests/)를
참고한다.
...
<footer>
  <button id="dl-button">JSON으로 데이터 다운로드</button>
  <div class="spacer"></div>
  <div class="small">제공: <a rel="noopener"
