...
title: 오픈텔레메트리란 무엇인가?
description:
  오픈텔레메트리(OpenTelemetry)가 무엇이고 무엇이 아닌지에 대한 간략한 설명이다.
...
- **오픈소스**이며 **벤더 및 툴 중립적**이다. 즉 [Jaeger][]나 [Prometheus][]와
  같은 오픈소스 도구를 포함해 다양한 옵저버빌리티 백엔드는 물론 상용 제품과도
  함께 사용할 수 있다. 오픈텔레메트리는 그 자체로 옵저버빌리티 백엔드가
  **아니다**.
...
- [Kubernetes용 오픈텔레메트리 오퍼레이터](../platforms/kubernetes/operator/),
...
다양한 [라이브러리, 서비스, 애플리케이션](/ecosystem/integrations/)에서 사용되고
있다.

오픈텔레메트리는 수많은 [벤더](/ecosystem/vendors/)의 지원을 받고 있으며, 이들
중 다수는 오픈텔레메트리에 대한 상용 지원을 제공하고 프로젝트에 직접 기여하고
있다.
...
- 아직 오픈텔레메트리 프로토콜(OTLP)을 지원하지 않는 사용자 지정 백엔드를 위한
  새 익스포터를 만드는 것
- 비표준 컨텍스트 전파 형식을 위한 사용자 지정 전파자를 만드는 것
