...
이루어져 있으며, 그중 일부는 언어별로 다르고 일부는 언어에 독립적이다.
...
[언어 API 또는 SDK](/docs/languages/)의 개발 상태, 즉 성숙도 수준에 대해서는
다음 표를 참고한다.
...
컬렉터의 상태는 핵심 컬렉터 구성 요소들이 현재 서로 다른
[안정성 수준](https://github.com/open-telemetry/opentelemetry-collector#stability-levels)을
가지고 있어 [혼합(mixed)](/docs/specs/otel/document-status/#mixed)이다.
...
오픈텔레메트리 오퍼레이터의 상태는 서로 다른 상태의 구성 요소를 배포하기 때문에
[혼합(mixed)](/docs/specs/otel/document-status/#mixed)이다.
...
[명세](/docs/specs/otel/)의 개발 상태, 즉 성숙도 수준에 대해서는 다음을
참고한다: [명세 상태 요약](/docs/specs/status/).
